...
async function tryAcquireLeadership() {
  try {
    const result =
      await prisma.$queryRaw`SELECT pg_try_advisory_lock(${ADVISORY_LOCK_ID}) AS acquired`;
...
async function cleanExpiredLocks() {
  ...
  try {
    ...
    logger.info(
      `Found ${expiredSeats.length} expired seat lock(s) to clean up`,
    );
    ...
    for (const [scheduleId, seats] of Object.entries(bySchedule)) {
      try {
        ...
        logger.error(
          `Failed to clean expired locks for schedule ${scheduleId}`,
          {
            error: err.message,
          },
        );
